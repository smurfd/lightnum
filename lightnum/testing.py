...
class testing:
  def looper_assert(self, x: Union[int, List[Any], SupportsAbs[float]], y: Union[int, List[Any], SupportsAbs[float]], close: bool=False, cls: bool=False) -> List[Any[int, float]]:
    ...
    return ret
  def looper_assert_close(self, x: Union[Any, List[Any]], y: Union[Any, List[Any]]) -> List[Any[int, float]]: return self.looper_assert(x, y, close=True)
  ...
  def assert_equal(self, x: Union[Any, List[Any[int, float]]], y: Union[List[Any[int, float]], Any]) -> None:
  ...
  def assert_array_equal(self, x: Union[Any, List[Any[int, float]]], y: Union[Any, List[Any[int, float]]]) -> None:
    if not builtins.all(self.looper_assert(x, y)): raise AssertionError("Values in array are not equal")

  def assert_allclose(self, x: Union[Any, List[Any]], y: Union[Any, List[Any]], rtol: float=1e-07, atol: int=0) -> bool:
    miss, r, rdiff, adiff = 0, [], [], []
    if builtins.all(isinstance(i, int) for i in [x,y]):
      if x != y:
        miss += 1
        if atol: adiff.append(abs(abs(x) - abs(y))) #type: ignore
        rdiff.append(abs(abs(x) - abs(y)) / abs(y)) #type: ignore
    else:
  ...
  def timing_test_looper(self, fn1: Callable[..., Any], fn2: Callable[..., Any], s: str, *args: Any, **kwargs: Any) -> None:
