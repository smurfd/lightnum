...
class testing:
  ...
  def looper_assert_close(x, y):
    ret=[]
    for i in range(len(x)):
      if isinstance(y, list) and builtins.all(isinstance(j, list) for j in [x[i],y[i]]): ret = testing.looper_assert_close(x[i], y[i])
      elif isinstance(x[i], list) and not isinstance(y, list): ret = testing.looper_assert_close(x[i], y)
      elif isinstance(x[i], tuple): ret = testing.looper_assert_close(x[i], y)
  ...
  def looper_assert_close_cls(x, y):
    ret=[]
    for i in range(len(x)):
      if isinstance(y, list) and builtins.all(isinstance(j, list) for j in [x[i],y[i]]): ret = testing.looper_assert_close_cls(x[i], y[i])
      elif isinstance(x[i], list) and not isinstance(y, list): ret = testing.looper_assert_close_cls(x[i], y)
      elif isinstance(x[i], tuple): ret = testing.looper_assert_close_cls(x[i], y)
      else: ret = testing.if_round_abs(x[i], y[i],cls=True)#, ret, cls=True)
    return ret
