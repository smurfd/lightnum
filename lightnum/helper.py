...
class helper:
  ...
  def looper_flip(self, x: List[Any], dtype: Type[Any] = int32) -> List[Any]: return [self.looper_flip(i) for i in x] if isinstance(x[0], list) else x[::-1]
  def looper_squeeze(self, x, axis=0) -> List[Any]: return x if not isinstance(x[0][0], list) else [self.looper_squeeze(y, axis) for y in x].pop() #TODO: axis
  def looper_clip(self, x, x_min, x_max) -> float|int: return (x_min if x < x_min else x_max) if not isinstance(x, list) else [self.looper_clip(y, x_min, x_max) for y in x]
  def looper_broadcast_to(self, x, y) -> List[Any]: return x if len(y) == 1 else [x for i in range(y[0])]
  def looper_nonzero(self, x):
    ret1, ret2 = [], []
  ...
  def looper_split(self, x, y, dtype=int32):
  ...
  def looper_tile(self, x, y, dtype=int32):
    tmp, tmp2=[], []
  ...
  def looper_empty(self, x, fill, dtype=int32):
  ...
  def looper_empty_like(self, x, fill, dtype=int32):
  ...
  # BARF
  def looper_pad(self, x, y, mode='constant', **kwargs):
    ret = []
  ...
  def read_magic(self, f): return f.read(len(self.MAGIC_PREFIX)) == self.MAGIC_PREFIX
  def read_header_type(self, f): return int.from_bytes(f.read(1), "big"), int.from_bytes(f.read(1), "big")
  def read_header_pack(self, t): return self._header_size_info.get(t)[0]
  def read_header_enc(self, t): return self._header_size_info.get(t)[1]
  def read_header_len(self, f, p): return struct.unpack(self.read_header_pack(p), f.read(2))[0]
  def read_header(self, f, t):
    import ast
    h = f.read(self.read_header_len(f, t)).decode(self.read_header_enc(t))
    return ast.literal_eval(h)['shape'], ast.literal_eval(h)['descr']
  def read_body(self, f, l): return [int.from_bytes(f.read(8), "little") for _ in range(0, l, 8)]

  def write_magic(self, f): f.write(self.MAGIC_PREFIX)
  def write_header_type(self, f, t):
    f.write(t[0].to_bytes(1, byteorder ='big'))
    f.write(t[1].to_bytes(1, byteorder ='big'))
  def write_header_len(self, f, p, header): f.write(struct.pack('<H', len(header)))
  def write_header(self, f, t, header): f.write(header.encode())
  def write_body(self, f, x): [f.write(i.to_bytes(8,'little')) for i in x]
