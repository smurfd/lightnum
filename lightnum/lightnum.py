...
def empty(x, fill=0, dtype=dtype): return helper.looper_empty(x, fill=fill, dtype=dtype)
...
def ones(x, fill=1, dtype=int32): return zeros(x, fill, dtype=dtype)
...
def promote_types(x, y): return dtype2(x) if dtype2(x) >= dtype2(y) else dtype2(y)
